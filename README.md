# OlegGlushchenko_infra_11
- Установлен Vagrant. Созданы виртуальные машины dbserver и appserver, проверена их работоспособность.
- В vagrant добавлен ansible в качестве провижинера. Доработаны роли для dbserver'а и appserver'a. В ходе доработок:
    - создан yml для установки python
    - созданы yml для установки и настройки mongo для dbserver
    - параметризировали роль для appserver
    - переопределили переменные (необходимо сменить имя пользователя для deploy)
- Выполнено задание со *. Необходимо дописать в секцию extra_vars переменные для запуска nginx'а. Переменные берутся из предыдущей практики
- Провели тестирование роли. Для этого:
    - через virtualenv создали виртуальную среду (созданную папку вируальной среды добавили в gitignore) 
    - установили через pip зависимости: Molecule, Ansible, Testinfra
    - создали заготовки тестов для роли db, создали тестовую машину, прогнали дефолтные тесты.
- Написан тест к роли db для проверки порта 27017
- Роли db и app переиспользованы в плейбуках packer_db.yml и packer_db.yml. Путь до ролей прописан.
